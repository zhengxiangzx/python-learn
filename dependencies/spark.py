...
def gci(filepath):
    print('root'.center(20, '-'), filepath)
    files = listdir(filepath)
    for fi in files:
        fi_d = path.join(filepath, fi)
        print('gci'.center(20, '-'), path.join(filepath, fi_d))
...
def start_spark(app_name='my_spark_app', master='local[*]', jar_packages=[],
    ...
    print('flag'.center(20, '-'), flag_repl, flag_debug)
    ...
    # --deploy-mode cluster 到worker目录寻找
    if len(config_files) == 0:
        spark_logger.warn('spark files dir not found config file, query worker dir')
        spark_files_dir = getcwd()
        config_files = [filename
                        for filename in listdir(spark_files_dir)
                        if filename.endswith('config.json')]
